...
def make_reconstruction(KK, csd_profile, csd_seed, total_ele,
    ...
    k, est_csd = KK.do_kcsd(pots, ele_pos, method=method, Rs=Rs,
                            lambdas=lambdas)
#    test_csd = csd_profile([k.estm_x, k.estm_y], KK.csd_seed)
#    rms = KK.calculate_rms(test_csd, est_csd[:, :, 0])
#    title = "Lambda: %0.2E; R: %0.2f; RMS: %0.2E; CV_Error: %0.2E; "\
#            % (k.lambd, k.R, rms, k.cv_error)
#    KK.make_plot(csd_at, true_csd, test_csd, k, est_csd, ele_pos,
#                 pots, title)
#    point_error = KK.calculate_point_error(test_csd, est_csd[:, :, 0])
...
def make_subplot(ax, val_type, xs, ys, values, cax, title=None, ele_pos=None,
    ...
    if mask is not False:
        ax.contourf(xs, ys, mask, alpha=0.3, cmap='Greys')
    if val_type == 'pot':
...
#    gs.update(top=.95, bottom=0.53)
    ...
    make_subplot(ax, 'diff', csd_x, csd_y, abs(true_csd-k.values('CSD')[:, :, 0]), cax=cax, ele_pos=ele_pos,
                 ...
                 t_max=np.max(abs(true_csd)), level=np.linspace(0, np.max(abs(true_csd)), 16))
