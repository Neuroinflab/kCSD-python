...
def do_kcsd(CSD_PROFILE, csd_seed, noise_level):
    if CSD_PROFILE.__name__ == "gauss_2d_small":
        ...
        R_final = np.linspace(0.05, 1.0, 20)
    ...
    noise = (
        noise_level
        * 0.01
        * (rstate.normal(np.mean(pots), np.std(pots), size=(len(pots), 1)))
    )
    pots += noise
...
def generate_figure(small_seed, large_seed):
    ...
    t_max_1 = 0.75
    csd_x, csd_y, err, ele_pos = do_kcsd(
        CSD.gauss_2d_small, csd_seed=small_seed, noise_level=0
    )
    ...
    csd_x, csd_y, err, ele_pos = do_kcsd(
        CSD.gauss_2d_small, csd_seed=small_seed, noise_level=5
    )
    ...
    csd_x, csd_y, err, ele_pos = do_kcsd(
        CSD.gauss_2d_small, csd_seed=small_seed, noise_level=10
    )
    ...
    csd_x, csd_y, err, ele_pos = do_kcsd(
        CSD.gauss_2d_small, csd_seed=small_seed, noise_level=30
    )
    ...
    csd_x, csd_y, err, ele_pos = do_kcsd(
        CSD.gauss_2d_large, csd_seed=large_seed, noise_level=0
    )
    ax = plt.subplot(gs[0, 0])
    cax = plt.subplot(gs[1, 0])
    make_subplot(
    ...
    csd_x, csd_y, err, ele_pos = do_kcsd(
        CSD.gauss_2d_large, csd_seed=large_seed, noise_level=5
    )
    ax = plt.subplot(gs[0, 1])
    cax = plt.subplot(gs[1, 1])
    make_subplot(
    ...
    csd_x, csd_y, err, ele_pos = do_kcsd(
        CSD.gauss_2d_large, csd_seed=large_seed, noise_level=10
    )
    ax = plt.subplot(gs[0, 2])
    cax = plt.subplot(gs[1, 2])
    make_subplot(
    ...
    csd_x, csd_y, err, ele_pos = do_kcsd(
        CSD.gauss_2d_large, csd_seed=large_seed, noise_level=30
    )
    ax = plt.subplot(gs[0, 3])
    cax = plt.subplot(gs[1, 3])
    make_subplot(
