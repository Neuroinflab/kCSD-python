...
def dan_make_plot(k):
    ...
    make_plot(
        ax1, k.estm_x, k.estm_y, est_csd[:, :], title="Estimated CSD", cmap=cm.bwr
    )

    ax2 = plt.subplot(122)
    make_plot(
        ax2, k.estm_x, k.estm_y, est_pots[:, :], title="Estimated POT", cmap=cm.PRGn
    )
...
tStart, tEnd = 0.0, 600.0  # 0., 600.  # 0., 1.        # in seconds
...
k.L_curve(Rs=np.logspace(-1.0, 2.0, 5), lambdas=None)
...
for h in 1.0, 4.0, 16.0, 32.0, 64.0, 128.0:
    k = KCSD2D(
        ele_pos,
        pots,
        h=h,
    ...
    k.L_curve(Rs=np.logspace(-1.0, 2.0, 11))
    plt.imshow(k.curve_surf)
    dan_make_plot(k)
