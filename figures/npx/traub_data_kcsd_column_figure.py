...
def fetch_mid_pts(h, pop_name):
    ...
    x = (all_pts["x0"] + all_pts["x1"]) / 2.0
    y = (all_pts["y0"] + all_pts["y1"]) / 2.0
    z = (all_pts["z0"] + all_pts["z1"]) / 2.0
...
def get_all_src_pos(h, pop_names, total_cmpts):
    ...
    for jj, pop_name in enumerate(pop_names):
        all_srcs[
            np.sum(total_cmpts[:jj], dtype=int) : np.sum(
                total_cmpts[: jj + 1], dtype=int
            ),
            :,
...
def plot_morp_ele(ax1, src_pos, ele_pos, pot, time_pt):
    ...
    plt.scatter(src_pos[:, 0], src_pos[:, 1], marker=".", alpha=0.7, color="k", s=0.7)
    plt.scatter(ele_pos[:, 0], ele_pos[:, 1], marker="x", alpha=0.8, color="r", s=10)
    # for tx in range(len(ele_pos[:,0])):
    #    plt.text(ele_pos[tx, 0], ele_pos[tx, 1], str(tx))
    try:
        ele_1 = 152
        ele_2 = 148
        plt.scatter(ele_pos[ele_1, 0], ele_pos[ele_1, 1], marker="s", color="m", s=30.0)
        plt.scatter(ele_pos[ele_2, 0], ele_pos[ele_2, 1], marker="s", color="b", s=30.0)
    except IndexError:
        ele_1 = 10
        ele_2 = 15
        plt.scatter(ele_pos[ele_1, 0], ele_pos[ele_1, 1], marker="s", color="m", s=30.0)
    ...
    plt.plot(
        np.zeros_like(dummy_line) + time_pt, dummy_line, color="black", linewidth=1
    )
...
def plot_extracellular(
    ax, lfp, ele_pos, num_x, num_y, time_pt, name, letter="", title=""
):
        ...
        for ii in range(nX):
            for jj in range(nY):
                csd_smoothed[ii, jj] += (
                    ...
                    * np.exp(
                        -(
                            (linX[ii] - X[i_source]) ** 2
                            + (linY[jj] - Y[i_source]) ** 2
                            + (csd_smoothed_Z - Z[i_source]) ** 2
                        )
                        / denominator
                    )
                )
                normalization[ii, jj] += A * np.exp(
                    -(
                        (linX[ii] - X[i_source]) ** 2
                        + (linY[jj] - Y[i_source]) ** 2
                        + (csd_smoothed_Z - Z[i_source]) ** 2
...
def plot_all_currents(
    ax, xmin, xmax, ymin, ymax, all_x, all_y, all_z, all_val, letter="", title=""
):
...
def plot_csd_slice(
    ...
    vx, vy = np.mgrid[-400 : 400 : complex(0, nx + 1), -2000 : 500 : complex(0, ny + 1)]
    ...
    for ii in range(nx):
        indexes_lower_x = np.array(
            np.where(np.array(all_x) < vx[ii + 1, 0])[0], dtype=int
        )
        indexes_higher_x = np.array(np.where(np.array(all_x) > vx[ii, 0])[0], dtype=int)
...
def plot_dense_potentials(
    ax, h, pop_names, time_pts, time_pt_interest, letter="", title="", filt=False
):
    # big 2d grid
    # nx, ny = 180, 600
...
def prepare_electrodes():
    ...
    od, do = 0, -(10 + 22.5) * ny
    xx, yy = np.mgrid[
        -dist_from_center_axis : dist_from_center_axis : complex(0, nx),
        od : do : complex(0, ny),
    ]
    xx = xx.reshape(tot_ele, 1)
    yy = yy.reshape(tot_ele, 1)
    ...
    x1, y1 = np.mgrid[
        left_border : left_border + 2 * dist_h : complex(0, nx / 2),
    ...
    x2, y2 = np.mgrid[
        left_border + dist_h : left_border + 3 * dist_h : complex(0, nx / 2),
...
def plot_csd_smooth(
    ax, xmin, xmax, ymin, ymax, csd, XX, YY, letter="", title="", ele_pos=None
):
    ...
    try:
        ax.scatter(ele_pos[:, 0], ele_pos[:, 1], marker="s", alpha=0.3, color="m", s=10)
...
def make_column_plot(
    ...
    plot_all_currents(
        ax1, xmin, xmax, ymin, ymax, all_x, all_y, all_z, all_val, letter="A"
    )

    ax2 = plt.subplot(242, aspect="equal")
    plot_csd_slice(
        ax2, xmin, xmax, ymin, ymax, all_x, all_y, all_z, all_val, letter="B"
    )

    ax3 = plt.subplot(243, aspect="equal")
    plot_dense_potentials(
        ax3, h, pop_names, time_pts, time_pt_interest, letter="C", filt=False
    )

    ax4 = plt.subplot(244, aspect="equal")
    xx, yy = np.mgrid[
        xmin : xmax : complex(0, true_csd.shape[0]),
        ymin : ymax : complex(0, true_csd.shape[1]),
    ]
    plot_csd_smooth(ax4, xmin, xmax, ymin, ymax, true_csd[:, :], xx, yy, letter="D")
    ...
    for i in range(len(elec_pos_list)):
        ...
        kcsd, est_pot, x, y, k = do_kcsd(
            pot, ele_pos[:, :2], xmin, xmax, ymin, ymax, n_src_init=5000
        )
        plot_csd_smooth(
            ax_list1[i],
            xmin,
        ...
        plot_csd_smooth(
            ...
            ymin,
            ymax,
            projection.reshape(true_csd_p.shape),
            x,
            y,
...
if __name__ == "__main__":
    ...
    all_x, all_y, all_z, all_val = extract_csd_timepoint(
        h, pop_names, time_pt_interest, "i"
    )
