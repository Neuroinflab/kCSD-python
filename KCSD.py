...
using the kCSD method from Potworowski et.al (2012).
...
class KCSD(CSD):
    ...
    electrode positions and recorded potentials. The method implemented here
    is based on the original paper by Jan Potworowski et al. (2012).
    """
    ...
    def create_lookup(self, dist_table_density=100):
        ...
        Updates and Returns the potentials due to a given basis source
        like a lookup table, whose shape=(dist_table_density,)-->
        set in KCSD2D_Helpers.py
...
#                                                       kind='cubic')
        self.interpolate_pot_at = \
            interpolate.InterpolatedUnivariateSpline(xs, dist_table, k=3)
        return
    ...
    def values(self, estimate='CSD'):
        ...
        k_inv = scipy.linalg.inv(self.k_pot + self.lambd *
        ...
        for t in range(self.n_time):
            ...
            for i in range(self.n_ele):
                estimation[:, t] += estimation_table[:, i] * \
                    beta[i]  # C*(x) Eq 18
    ...
    def process_estimate(self, estimation):
        if self.dim == 1:
            ...
            estimation = estimation.reshape(
                self.ngx, self.ngy, self.ngz, self.n_time)
        return estimation

    def find_min_nonzero_element(self, *elements, **at_least):
        at_least = at_least.pop('at_least', 1e-12)
        mins = []
        for element in elements:
            try:
                mins.append(min(element[element > 0.]))
            except TypeError:
                pass

        return max(np.min(mins), at_least)
    ...
    def update_R(self, R):
        ...
        self.dist_min = self.find_min_nonzero_element(self.src_ele_dists,
                                                      self.src_estm_dists)
        self.method()
        return
    ...
    def compute_cverror(self, lambd, index_generator):
        ...
        for idx_train, idx_test in index_generator:
            ...
            try:
                ...
                print('Encountered Singular Matrix Error: try changing'
                      'ele_pos')
