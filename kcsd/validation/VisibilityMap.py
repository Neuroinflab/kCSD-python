...
class VisibilityMap1D(ValidateKCSD1D):
    ...
    def calculate_error_map(
        ...
        if PARALLEL_AVAILABLE:
            err = Parallel(n_jobs=NUM_CORES)(
                delayed(self.make_reconstruction)(
                    csd_profile,
                    i,
                    noise=noise,
            ...
            point_error = []
            for i in range(n):
                data, error = self.make_reconstruction(
                    csd_profile,
                    i,
                    noise,
                    nr_broken_ele,
    ...
    def plot_error_map(self, point_error, ele_pos):
        ...
        plt.plot(
            np.linspace(self.kcsd_xlims[0], self.kcsd_xlims[-1], mean_err.shape[0]),
            mean_err,
            "b.",
            label="mean error",
        )
...
class VisibilityMap2D(ValidateKCSD2D):
    ...
    def plot_error_map(self, point_error, ele_pos):
        ...
        x, y = np.mgrid[
            self.kcsd_xlims[0] : self.kcsd_xlims[1] : complex(0, point_error.shape[1]),
            self.kcsd_ylims[0] : self.kcsd_ylims[1] : complex(0, point_error.shape[2]),
        ]
...
class VisibilityMap3D(ValidateKCSD3D):
    ...
    def plot_error_map(self, point_error, ele_pos):
        ...
        x, y, z = np.mgrid[
            self.kcsd_xlims[0] : self.kcsd_xlims[1] : complex(0, point_error.shape[1]),
            self.kcsd_ylims[0] : self.kcsd_ylims[1] : complex(0, point_error.shape[2]),
            self.kcsd_zlims[0] : self.kcsd_zlims[1] : complex(0, point_error.shape[3]),
        ]
        mean_error = self.sigmoid_mean(point_error)
        ...
        gs = gridspec.GridSpec(
            z_steps, 2, height_ratios=height_ratios, width_ratios=width_ratios
        )
        levels = np.linspace(0, 1.0, 25)
...
if __name__ == "__main__":
    ...
    rms, point_error = k.calculate_error_map(CSD_PROFILE, Rs=np.arange(0.2, 0.5, 0.1))
    ...
    rms, point_error = a.calculate_error_map(
        CSD_PROFILE, Rs=np.arange(0.05, 0.5, 0.05), lambdas=np.array(0), n=2
    )

    print("Checking 2D MoI")
    ELE_LIMS = [0.118, 0.882]
    CSD_PROFILE = CSD.gauss_2d_large
    a = VisibilityMap2DMoI(
        total_ele=9,
        h=50.0,
        sigma=1.0,
        n_src_init=1000,
    ...
    a = VisibilityMap3D(total_ele=64, h=50.0, sigma=1.0, n_src_init=729)
