...
def distribute_srcs_1D(X, n_src, ext_x, R_init):
    ...
    """
    X_src = np.mgrid[(np.min(X) - ext_x):(np.max(X) + ext_x):np.complex(0,
                                                                        n_src)]
    R = R_init
    return X_src, R
...
def distribute_srcs_2D(X, Y, n_src, ext_x, ext_y, R_init):
    ...
    ext_x_n = old_div((Lx_nn - Lx), 2)
    ext_y_n = old_div((Ly_nn - Ly), 2)
    X_src, Y_src = np.mgrid[(np.min(X) -
                             ext_x_n):(np.max(X) +
                                       ext_x_n):np.complex(0, nx),
                            (np.min(Y) -
                             ext_y_n):(np.max(Y) +
                                       ext_y_n):np.complex(0, ny)]
    d = round(old_div(R_init, ds))
    R = d * ds
...
def get_src_params_2D(Lx, Ly, n_src):
    ...
    rts = np.roots(coeff)
    r = [r for r in rts if not isinstance(r, complex) and r > 0]
    nx = r[0]
    ny = old_div(n_src, nx)
    ds = old_div(Lx, (nx - 1))
    nx = np.floor(nx) + 1
...
def distribute_srcs_3D(X, Y, Z, n_src, ext_x, ext_y, ext_z, R_init):
    ...
    ext_x_n = old_div((Lx_nn - Lx), 2)
    ext_y_n = old_div((Ly_nn - Ly), 2)
    ext_z_n = old_div((Lz_nn - Lz), 2)
    X_src, Y_src, Z_src = np.mgrid[(np.min(X) -
                                    ext_x_n):(np.max(X) +
                                              ext_x_n):np.complex(0, nx),
                                   (np.min(Y) -
                                    ext_y_n):(np.max(Y) +
                                   ...
                                   (np.min(Z) -
                                    ext_z_n):(np.max(Z) +
                                              ext_z_n):np.complex(0, nz)]
    d = np.round(old_div(R_init, ds))
    R = d * ds
...
def get_src_params_3D(Lx, Ly, Lz, n_src):
    ...
    """
    V = Lx * Ly * Lz
    V_unit = old_div(V, n_src)
    L_unit = V_unit**(old_div(1., 3.))
    nx = np.ceil(old_div(Lx, L_unit))
    ny = np.ceil(old_div(Ly, L_unit))
    nz = np.ceil(old_div(Lz, L_unit))
    ds = old_div(Lx, (nx - 1))
    Lx_n = (nx - 1) * ds
    Ly_n = (ny - 1) * ds
    Lz_n = (nz - 1) * ds
