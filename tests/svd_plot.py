...
def picard_plot(kernel, pots, sigma, u):
#    b = np.dot(kernel, pots[:, 0])
    b = pots
    print(b.shape)
...
#        picard[i] = np.linalg.norm(np.dot(u[:, i].reshape(len(u[:, i]), 1), b.reshape((len(pots), 1)).T), ord=1)
#        picard_norm[i] = np.linalg.norm(u[:, i].T*b, ord=1)/sigma[i]
        picard_norm[i] = abs(np.dot(u[:,i].T, b))/sigma[i]
    return picard, picard_norm
...
picard, picard_norm = picard_plot(k_pot, pots, s, U)
plt.figure()
