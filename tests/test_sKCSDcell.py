...
from kcsd.sKCSD import sKCSDcell
...
class testsKCDcell(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        n_src = 1000
        cls.cell = sKCSDcell(cls.data.morphology,
                             cls.data.ele_pos/sc,
                             n_src)
        ...
        cls.cell_small = sKCSDcell(data.morphology,
                                   data.ele_pos/sc,
                                   10)
        cls.cell_small.distribute_srcs_3D_morph()
        cls.small_loops_seg_coor = cls.cell_small.coordinates_3D_loops()
        cls.small_points = np.zeros((len(cls.cell_small.morphology), ))
        dic = cls.small_loops_seg_coor
        for seg in dic:
            ps = dic[seg]
            for p in ps:
                cls.small_points[p[2]] += 1
        cls.small_seg_coor = cls.cell_small.coordinates_3D_segments()
        # Y-shaped neuron
        data = Data("Data/Y_shaped_neuron")
        data.morphology[:, 2:6] = data.morphology[:, 2:6]/sc
        cls.cell_y = sKCSDcell(data.morphology,
                               data.ele_pos/sc,
                               10)
        cls.cell_y.distribute_srcs_3D_morph()
        cls.y_loop_seg_coor = cls.cell_y.coordinates_3D_loops()
        cls.y_points = {}
        dic = cls.y_loop_seg_coor
        for seg in dic:
            ps = dic[seg]
            for p in ps:
                s = '%d%d%d' % (p[0], p[1], p[2])
    ...
    def test_calculate_total_distance_max_dist_small(self):
        ...
        max_dist = self.cell_small.max_dist
        tot_dist = self.cell_small.calculate_total_distance()
        self.assertTrue(np.isclose(max_dist, tot_dist))

    def test_calculate_total_distance_max_dist(self):
        # self.max_dist = self.est_pos.max()# add test
        max_dist = self.cell.max_dist
        tot_dist = self.cell.calculate_total_distance()
        self.assertTrue(np.isclose(max_dist, tot_dist))

    def test_max_est_pos_total_distance_small(self):
        # self.max_dist = self.est_pos.max()# add test
        max_dist = self.cell_small.max_dist
        est_pos_max = self.cell_small.est_pos.max()
        self.assertTrue(np.isclose(max_dist, est_pos_max))

    def test_max_est_pos_total_distance(self):
        # self.max_dist = self.est_pos.max()# add test
        max_dist = self.cell.max_dist
        est_pos_max = self.cell.est_pos.max()
        self.assertTrue(np.isclose(max_dist, est_pos_max))

    def test_get_xyz_x_small(self):
        a = in_between(self.cell_small.est_pos[1, 0],
                       self.cell_small.est_pos[2, 0])
        b = in_between(self.cell_small.est_xyz[1, 0],
                       self.cell_small.est_xyz[2, 0])
    ...
    def test_get_xyz_y_small(self):
        a = in_between(self.cell_small.est_pos[1, 0],
                       self.cell_small.est_pos[2, 0])
        b = in_between(self.cell_small.est_xyz[1, 1],
                       self.cell_small.est_xyz[2, 1])
        x, y, z = self.cell_small.get_xyz(a)
        self.assertTrue(y == b)
    ...
    def test_get_xyz_z_small(self):
        a = in_between(self.cell_small.est_pos[1, 0],
                       self.cell_small.est_pos[2, 0])
        b = in_between(self.cell_small.est_xyz[1, 2],
                       self.cell_small.est_xyz[2, 2])
        x, y, z = self.cell_small.get_xyz(a)
        self.assertTrue(np.isclose(z, b))

    def test_get_xyz_x(self):
        a = in_between(self.cell.est_pos[1, 0],
                       self.cell.est_pos[2, 0])
        b = in_between(self.cell.est_xyz[1, 0],
                       self.cell.est_xyz[2, 0])
    ...
    def test_get_xyz_y(self):
        a = in_between(self.cell.est_pos[1, 0],
                       self.cell.est_pos[2, 0])
        b = in_between(self.cell.est_xyz[1, 1],
                       self.cell.est_xyz[2, 1])
        x, y, z = self.cell.get_xyz(a)
    ...
    def test_get_xyz_z(self):
        a = in_between(self.cell.est_pos[1, 0],
                       self.cell.est_pos[2, 0])
        b = in_between(self.cell.est_xyz[1, 2],
                       self.cell.est_xyz[2, 2])
        x, y, z = self.cell.get_xyz(a)
        self.assertTrue(np.isclose(z, b))

    def test_segments_small(self):
        segm = len(self.cell_small.morphology) - 1
        self.assertTrue(self.cell_small.segment_counter == segm)

    def test_segments(self):
        segm = len(self.cell.morphology)-1
        self.assertTrue(self.cell.segment_counter == segm)
    ...
    def test_get_grid_small_z(self):
        m_len = len(self.cell_small.morphology)
        self.assertTrue(self.cell_small.dims[2] == m_len)
    ...
    def test_get_grid_large_z(self):
        if self.cell.dxs[2]:
            dim_no = (self.cell.zmax - self.cell.zmin)/self.cell.dxs[2]
            self.assertTrue(self.cell.dims[2] == 1 + np.floor(dim_no))
        else:
            self.assertTrue(self.cell.dims[2] == 1)

    def test_get_grid_large_y(self):
        if self.cell.dxs[1]:
            dim_no = (self.cell.ymax - self.cell.ymin)/self.cell.dxs[1]
            self.assertTrue(self.cell.dims[1] == 1 + np.floor(dim_no))
        else:
            self.assertTrue(self.cell.dims[1] == 1)

    def test_get_grid_large_x(self):
        if self.cell.dxs[2]:
            dim_no = (self.cell.xmax - self.cell.xmin)/self.cell.dxs[0]
            self.assertTrue(self.cell.dims[0] == 1 + np.floor(dim_no))
        else:
            self.assertTrue(self.cell.dims[0] == 1)

    def test_dxs_small_x(self):
        first = self.cell_small.dxs[0] > self.cell_small.tolerance
        second = self.cell_small.dxs[0] == 0
        self.assertTrue(first or second)

    def test_dxs_small_y(self):
        first = self.cell_small.dxs[1] > self.cell_small.tolerance
        second = self.cell_small.dxs[1] == 0
        self.assertTrue(first or second)

    def test_dxs_small_z(self):
        first = self.cell_small.dxs[2] > self.cell_small.tolerance
        second = self.cell_small.dxs[2] == 0
        self.assertTrue(first or second)

    def test_point_coordinates_morpho_x(self):
        other_zero = (self.morpho[0, 0] - self.cell.xmin)/self.cell.dxs[0]
        self.assertTrue(self.zero[0] == np.floor(other_zero))

    def test_point_coordinates_morpho_y(self):
        other_zero = (self.morpho[0, 1] - self.cell.ymin)/self.cell.dxs[1]
        self.assertTrue(self.zero[1] == np.floor(other_zero))

    def test_point_coordinates_morpho_z(self):
        other_zero = (self.morpho[0, 2] - self.cell.zmin)/self.cell.dxs[2]
        self.assertTrue(self.zero[2] == np.floor(other_zero))
    ...
    def test_except_ending_at_least_twice_small(self):
        # test coordinates 3D loops is every point
        # except ending at least twice small
    ...
    def test_coordinates_3D_loops_last_loop_two_counts(self):
        l = len(self.small_loops_seg_coor)-1
        self.assertTrue(len(self.small_loops_seg_coor[l]) == 2)

    def test_coordinates_3D_loops_last_loop_difference(self):
        l = len(self.small_loops_seg_coor)-1
        p0 = self.small_loops_seg_coor[l][0]
        p1 = self.small_loops_seg_coor[l][1]
        self.assertTrue(p0[2] == p1[2] + 1 or p0[2] == p1[2] - 1)

    def test_coor_3D_loops_every_but_last_loop_one_point(self):
        l = len(self.small_loops_seg_coor)
        for i in range(l-1):
            self.assertTrue(len(self.small_loops_seg_coor[i]) == 1)

    def test_coordinates_3D_loops_small_change(self):
        l = len(self.small_seg_coor)
        for i in range(1, l-1):
            a = self.small_seg_coor[i][0][2]
            b = self.small_seg_coor[i-1][0][2]
            self.assertTrue(a == b + 1 or a == b - 1)

    def test_coordinates_3D_loops_y_one_count(self):
        first = (self.y_points['64055'] == 1)
        second = (self.y_points['0055'] == 1)
    ...
    def test_coordinates_3D_small_length(self):
        l = len(self.small_seg_coor)
        l_seg = self.cell_small.morphology.shape[0] - 1
        self.assertTrue(l == l_seg)

    def test_coordinates_3D_y_length(self):
        l = len(self.cell_y_segment_coordinates)
        l_seg = self.cell_y.morphology.shape[0] - 1
        self.assertTrue(l == l_seg)

    def test_coordinates_3D_small_last(self):
        l = len(self.small_seg_coor)
        self.assertTrue(len(self.small_seg_coor[l-1]) == 2)
    ...
    def test_coordinates_3D_small_change(self):
        l = len(self.small_seg_coor)
        for i in range(1, l-1):
            first = self.small_seg_coor[i][0][2]
            second = self.small_seg_coor[i-1][0][2] + 1
            self.assertTrue(first == second)

    def test_coordinates_3D_loops_y_point_difference_x(self):
        l = len(self.y_loop_seg_coor) - 1
        d = self.y_loop_seg_coor
        for i in range(l):
            if len(d[i]) > 1:
                for j in range(1, len(d[i])):
                    ...
                    x11 = x1 + 1
                    x1_1 = x1 - 1
                    self.assertTrue(x0 == x1 or x0 == x11 or x0 == x1_1)
        ...
        for i in range(l):
            if len(d[i]) > 1:
                for j in range(1, len(d[i])):
                    ...
                    y11 = y1 + 1
                    y1_1 = y1 - 1
                    self.assertTrue(y0 == y1 or y0 == y11 or y0 == y1_1)
        ...
        for i in range(l):
            if len(d[i]) > 1:
                for j in range(1, len(d[i])):
                    ...
                    z11 = z1 + 1
                    z1_1 = z1 - 1
                    self.assertTrue(z0 == z1 or z0 == z11 or z0 == z1_1)
