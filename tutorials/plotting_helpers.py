...
def show_csd(csd_at, csd, show_ele=None, show_kcsd=False, show_mask=None):
    if config.dim == 1:
        ...
        if show_kcsd is False:
            ...
            im = ax.contourf(
                csd_at[0], csd_at[1], csd[:, :, 0], levels=levels, cmap=cm.bwr_r
            )
            ax.set_title("kCSD")
        ax.set_xlabel("Position mm")
        ...
        if show_mask is not None:
            ...
            im2 = ax.contourf(
                csd_at[0], csd_at[1], show_mask, levels=levels2, alpha=0.3, cmap="Greys"
            )
...
def show_pot(ele_pos, pot, no_ele=False):
    if config.dim == 1:
        ...
        ax.set_xlim([min(ele_x) - (0.1 * scale_x), max(ele_x) + (0.1 * scale_x)])
        ax.set_ylim([min(ele_y) - (0.1 * scale_y), max(ele_y) + (0.1 * scale_y)])
        ...
        for idx in range(min(5, ele_res)):
            X, Y, Z = grid(
                ele_x[:, :, idx].flatten(), ele_y[:, :, idx].flatten(), pot[:, :, idx]
            )
